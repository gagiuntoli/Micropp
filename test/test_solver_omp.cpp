	...
	class test_t : public micropp<3> {
		...
		public:
			...
			void just_do_it(const int size[3])
			{
				...
				double **du = (double**)malloc(nthreads * sizeof(double*));
				double **b = (double**)malloc(nthreads * sizeof(double*));
				ell_matrix *A = (ell_matrix*)malloc(nthreads * sizeof(ell_matrix));
				for (int i = 0; i < nthreads; ++i) {
					u[i] = (double*)malloc(nndim * sizeof(double));
					du[i] = (double*)malloc(nndim * sizeof(double));
					b[i] = (double*)malloc(nndim * sizeof(double));
...
#pragma omp parallel for
				for (int i = 0; i < REPETITIONS; ++i) {
					...
					double norm1 = assembly_rhs(u[thread_id], nullptr, b[thread_id]);
					assembly_mat(&A[thread_id], u[thread_id], nullptr);

					double cg_err;
					int cg_its = ell_solve_cgpd(&A[thread_id], b[thread_id], du[thread_id], &cg_err);

					for (int j = 0; j < nn * dim; ++j)
						u[thread_id][j] += du[thread_id][j];

					double norm2 = assembly_rhs(u[thread_id], nullptr, b[thread_id]);
					printf("Thread id : %d norm1 = %lf norm1 = %lf cg_its = %d cg_err = %lf\n", thread_id, norm1, norm2, cg_its, cg_err);
				}
