...
void micropp<2>::set_displ_bc(const double eps[nvoi], double *u) {
  const double eps_t[dim][dim] = {{eps[0], 0.5 * eps[2]}, {0.5 * eps[2], eps[1]}};
...
void micropp<2>::calc_bmat(int gp, double bmat[nvoi][npe * dim]) const {
  const double dsh[4][2] = {{-(1 - xg[gp][1]) / 4 * 2 / dx, -(1 - xg[gp][0]) / 4 * 2 / dy},
...
template <>
double micropp<2>::assembly_rhs(const double *u, const double *int_vars_old, double *b) {
  INST_START;
