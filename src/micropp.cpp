...
micropp<tdim>::micropp(const micropp_params_t &params)
    :
      ...
      micro_type(params.type),
      nvars(nelem * npe * NUM_VAR_GP),
  ...
  for (int gp = 0; gp < ngp; ++gp) {
    ...
    if (params.coupling == nullptr || (params.coupling[gp] == FE_ONE_WAY || params.coupling[gp] == FE_FULL)) {
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  for (int gp = 0; gp < ngp; ++gp) {
    if (gp_list[gp].coupling == FE_LINEAR || gp_list[gp].coupling == FE_ONE_WAY || gp_list[gp].coupling == FE_FULL) {
...
int micropp<tdim>::get_elem_type(int ex, int ey, int ez) const {
  const double coor[3] = {ex * dx + dx / 2., ey * dy + dy / 2., ez * dz + dz / 2.};  // 2D -> dz = 0
  ...
  if (micro_type == MIC_HOMOGENEOUS) {  // Only one material (mat[0])
    ...
    if (point_inside_cilinder_inf(dir_z, cen_1, rad, coor) || point_inside_cilinder_inf(dir_x, cen_2, rad, coor)) {
      return 1;
    }
    return 0;
    ...
    if (fabs(coor[0] - center[0]) < width && fabs(coor[1] - center[1]) < width) return 1;

    if (fabs(coor[0] - center[0]) < width && fabs(coor[2] - center[2]) < width) return 1;

    if (fabs(coor[1] - center[1]) < width && fabs(coor[2] - center[2]) < width) return 1;
    ...
    if (fabs(coor[1] - center[1]) < width && fabs(coor[2] - center[2]) < width &&
    ...
    const double centers[num_spheres][3] = {
        {.8663, .0689, .1568}, {.2305, .4008, .2093}, {.1987, .8423, .2126}, {.6465, .7095, .4446},
        {.6151, .9673, .2257}, {.1311, .9739, .4129}, {.8433, .0738, .2233}, {.5124, .2111, .0369},
        {.4094, .7030, .6241}, {.1215, .8289, .3812}, {.1125, .9266, .6872}, {.7422, .4030, .6000},
        {.0791, .3819, .9297}, {.1201, .7712, .0069}, {.9339, .2177, .1976}, {.3880, .1331, .9032},
        {.7319, .7146, .8150}, {.8796, .2858, .6701}, {.1427, .9309, .9830}, {.1388, .3126, .8054},
        {.0729, .4754, .4950}, {.5038, .7755, .7333}, {.0242, .2185, .9884}, {.5371, .5372, .8269},
        {.4564, .0039, .1715}, {.7410, .0799, .8775}, {.2627, .9047, .5681}, {.7894, .1908, .2993},
        {.1373, .5370, .9916}, {.0207, .8020, .9283}, {.6540, .2359, .0286}, {.7801, .3568, .5086},
        {.7322, .5706, .1114}, {.5479, .1493, .1267}, {.6722, .1530, .1003}, {.7659, .3426, .9181},
        {.4582, .4636, .6310}, {.1811, .9665, .4713}, {.0834, .6066, .6936}, {.4865, .7584, .3635}};

    const double rads[num_spheres] = {
        0.1 * .5741, 0.1 * .1735, 0.1 * .5065, 0.1 * .8565, 0.1 * .9735, 0.1 * .7087, 0.1 * .9585, 0.1 * .2843,
        0.1 * .4029, 0.1 * .2574, 0.1 * .1575, 0.1 * .3316, 0.1 * .1874, 0.1 * .6300, 0.1 * .7049, 0.1 * .7258,
        0.1 * .8002, 0.1 * .8176, 0.1 * .9970, 0.1 * .4736, 0.1 * .6351, 0.1 * .3140, 0.1 * .3440, 0.1 * .2444,
        0.1 * .3894, 0.1 * .5234, 0.1 * .1658, 0.1 * .4020, 0.1 * .6813, 0.1 * .2060, 0.1 * .1427, 0.1 * .9332,
        0.1 * .5406, 0.1 * .9765, 0.1 * .0956, 0.1 * .6432, 0.1 * .9998, 0.1 * .4166, 0.1 * .6907, 0.1 * .0404};
    ...
    const double dirs[num_fibs][3] = {{1, .5741, .8515}, {1, .1735, .1103}, {1, .5065, .4600}, {1, .8565, .9045},
                                      {1, .9735, .6313}, {1, .7087, .1547}, {1, .9585, .0220}, {1, .2843, .4062},
                                      {1, .4029, .8095}, {1, .2574, .4742}, {1, .1575, .0768}, {1, .3316, .0320},
                                      {1, .1874, .6364}, {1, .6300, .2688}, {1, .7049, .5137}, {1, .7258, .2799},
                                      {1, .8002, .4794}, {1, .8176, .4142}, {1, .9970, .2189}, {1, .4736, .6202}};
...
template <int tdim>
void micropp<tdim>::get_stress(int gp, const double eps[nvoi], const double *vars_old, double stress_gp[nvoi], int ex,
                               int ey, int ez) const {
