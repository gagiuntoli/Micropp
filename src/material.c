...
void material_set(struct material_base *self, const int _type, const double _E, const double _nu, const double _Ka,
...
void material_print(const struct material_base *self) {
  printf(
      "E = %e\nnu = %e\nKa = %e\nSy = %e\nXt = %e\nk = %e\nmu = %e\n"
      "lambda = %e\ntype = %1d\n",
      self->E, self->nu, self->Ka, self->Sy, self->Xt, self->k, self->mu, self->lambda, self->type);
}
