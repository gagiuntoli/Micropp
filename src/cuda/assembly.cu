...
__device__ void ell_add_3D_gpu(ell_matrix *m, double *vals_d, int ex, int ey, int ez, const double Ae[8 * 3 * 8 * 3]) {
...
__global__ void assembly_kernel(ell_matrix *A_d, double *vals_d, const double *u, int *elem_type_d, Params *params_d) {
        ...
        for (int gp = 0; gp < NPE; ++gp) {
          ...
          for (int i = 0; i < NVOI; ++i) {
            for (int j = 0; j < NPEDIM; ++j) {
              double tmp = 0.0;
              for (int k = 0; k < NVOI; ++k) tmp += ctan[i * NVOI + k] * params_d->bmat[gp][k][j];
              cxb[i][j] = tmp * wg;
            }
          }
...
__device__ void get_elem_rhs(const double *u, const double *vars_old, double be[NPEDIM], int *elem_type_d,
  ...
  for (int gp = 0; gp < NPE; ++gp) {
    ...
    for (int i = 0; i < NPEDIM; ++i)
      for (int j = 0; j < NVOI; ++j) be[i] += params_d->bmat[gp][j][i] * stress_gp[j] * params_d->wg;
  }
}

__global__ void assembly_rhs_kernel(double *b_d, const double *u, int *elem_type_d, Params *params_d) {
  const int nex = params_d->nex;
  const int ney = params_d->ney;
  const int nez = params_d->nez;
  ...
  for (int ex = ex_t; ex < nex; ex += stride_x) {
    for (int ey = ey_t; ey < ney; ey += stride_y) {
      for (int ez = ez_t; ez < nez; ez += stride_z) {
        ...
        for (int i = 0; i < NPE * DIM; ++i) atomicAdd_d(&b_d[index[i]], be[i]);
      }
    }
  }
}
