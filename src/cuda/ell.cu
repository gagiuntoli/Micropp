...
__global__ void ell_mvp_kernel(const ell_matrix *m_d, const double *vals_d, const int *cols_d, const double *x_d,
                               double *y_d) {
  ...
  for (int row = row_begin; row < m_d->nrow; row += stride) {
    const unsigned int ix = row * m_d->nnz + threadIdx.y;

    cache[blockDim.y * threadIdx.x + threadIdx.y] =
        (row < m_d->nrow && threadIdx.y < m_d->nnz) ? vals_d[ix] * x_d[cols_d[ix]] : 0;
    __syncthreads();

    for (unsigned int s = 1; s < blockDim.y; s *= 2) {
      if (threadIdx.y % (2 * s) == 0 && threadIdx.y < m_d->nnz) {
        cache[blockDim.y * threadIdx.x + threadIdx.y] += cache[blockDim.y * threadIdx.x + threadIdx.y + s];
      }
      __syncthreads();
    }
...
__global__ void get_diag_kernel(const ell_matrix *m_d, const double *vals_d, double *k_d) {
  int it = threadIdx.x + blockDim.x * blockIdx.x;
  int stride_x = blockDim.x * gridDim.x;

  for (int i = it; i < m_d->nn; i += stride_x) {
    for (int d = 0; d < m_d->nfield; ++d) {
      k_d[i * m_d->nfield + d] = 1 / vals_d[i * m_d->nfield * m_d->nnz + m_d->shift * m_d->nfield + d * m_d->nnz + d];
    }
  }
}

__global__ void axpby(const double alpha, const double *x, const double beta, double *y, const int n) {
  // y = alpha x + beta y
  int it = threadIdx.x + blockDim.x * blockIdx.x;
  int stride_x = blockDim.x * gridDim.x;
...
__global__ void dot_i(double *z, const double *x, const double *y, const int n) {
  int it = threadIdx.x + blockDim.x * blockIdx.x;
  int stride_x = blockDim.x * gridDim.x;
...
__global__ void dot_prod_kernel(const double *arr1_d, const double *arr2_d, double *g_res, const int n) {
...
__global__ void reduce(double *res_d, const double *arr_d, const int n) {
  __shared__ double sdata[1024];
  int it = threadIdx.x;
  int stride = blockDim.x;

  sdata[threadIdx.x] = 0;  // mv data to shared memory

  for (int i = it; i < n; i += stride) {
    sdata[threadIdx.x] += arr_d[i];
  }
  if (threadIdx.x == 0) {
    *res_d = 0.0;
...
int ell_solve_cgpd(const ell_matrix *m, const double *b, double *x, double *err) {
  ...
  const int grid_dot = 50000;
  const int block_dot = 128;
  ...
  dot_prod_kernel<<<grid_dot, block_dot>>>(r_d, z_d, g_res_d, m->nrow);
  reduce<<<1, 1024>>>(res_d, g_res_d, grid_dot);
  cudaMemcpy(&dot_h, res_d, sizeof(double), cudaMemcpyDeviceToHost);
  double rz = dot_h;

  dot_prod_kernel<<<grid_dot, block_dot>>>(z_d, z_d, g_res_d, m->nrow);
  reduce<<<1, 1024>>>(res_d, g_res_d, grid_dot);
  cudaMemcpy(&dot_h, res_d, sizeof(double), cudaMemcpyDeviceToHost);
  ...
  while (its < m->max_its) {
    ...
    dim3 grid_mvp(5000, 1, 1);
    dim3 block_mvp(1, 128, 1);
    ell_mvp_kernel<<<grid_mvp, block_mvp>>>(m_d, vals_d, cols_d, p_d, Ap_d);

    dot_prod_kernel<<<grid_dot, block_dot>>>(p_d, Ap_d, g_res_d, m->nrow);
    reduce<<<1, 1024>>>(res_d, g_res_d, grid_dot);
    cudaMemcpy(&dot_h, res_d, sizeof(double), cudaMemcpyDeviceToHost);
