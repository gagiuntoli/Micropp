    ...
    for (int ey = 0; ey < ney; ++ey) {
      for (int ex = 0; ex < nex; ++ex) {
        for (int gp = 0; gp < npe; ++gp) {
          get_strain(u, gp,
                     &strain_gp[ex * ney * nez * npe * nvoi + ey * nez * npe * nvoi + ez * npe * nvoi + gp * nvoi],
                     bmat, nx, ny, ex, ey, ez);
        }
      }
    }
  ...
  for (int ez = 0; ez < nez; ++ez) {
    for (int ey = 0; ey < ney; ++ey) {
      for (int ex = 0; ex < nex; ++ex) {
        ...
        for (int gp = 0; gp < npe; ++gp) {
          get_stress(gp, &strain_gp[ex * ney * nez * npe * nvoi + ey * nez * npe * nvoi + ez * npe * nvoi + gp * nvoi],
                     vars_old, stress_gp, ex, ey, ez);
...
#pragma acc parallel loop copy(eps[ : nex * ney * nez * npe * 6]) copyin(u[ : nndim])
  for (int ex = 0; ex < nex; ++ex) {
    for (int ey = 0; ey < ney; ++ey) {
      for (int ez = 0; ez < nez; ++ez) {
        for (int gp = 0; gp < npe; ++gp) {
          get_strain(u, gp, &eps[ex * ney * nez * npe * 6 + ey * nez * npe * 6 + ez * npe * 6 + gp * 6], bmat, nx, ny,
                     ex, ey, ez);
        }
      }
    }
      ...
      for (int ez = 0; ez < nez; ++ez) {
        ...
        for (int gp = 0; gp < npe; ++gp) {
          ...
          material->get_ctan(&eps[ex * ney * nez * npe * 6 + ey * nez * npe * 6 + ez * npe * 6 + gp * 6],
                             &ctan[ex * ney * nez * npe * nvoi * nvoi + ey * nez * npe * nvoi * nvoi +
                                   ez * npe * nvoi * nvoi + gp * nvoi * nvoi],
                             vars);
        }
  ...
  delete[] eps;
#pragma acc parallel loop gang vector copyin(                                                                 \
    ctan[ : nex * ney * nez * npe * nvoi * nvoi], bmat_arr[ : npe * nvoi * npedim], A[ : 1], A->nrow, A->nnz, \
    cols_row[ : 8][ : 8], ix_glo[ : nex * ney * nez * 8]) copy(A -> vals[ : A->nrow * A->nnz])
      ...
      for (int i = 0; i < nvoi; ++i) {
        for (int j = 0; j < npedim; ++j) {
          ...
          for (int k = 0; k < nvoi; ++k) {
            tmp += ctan[ex * npe * nvoi * nvoi + gp * nvoi * nvoi + i * nvoi + k] *
                   bmat_arr[gp * nvoi * npedim + k * npedim + j];
          }
          cxb[i][j] = tmp * wg;
        }
      }
...
#pragma acc atomic update
            A->vals[ix_glo[ex * 8 + i] * nnz_nfield + cols_row[i][j] * nfield + fi * nnz + fj] +=
